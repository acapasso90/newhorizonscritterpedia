...
export default function DisplaySeaCritters(){
 ...
 // Makes axios request for fish information then cancels request on cleanup function
    ...
    let mounted = true;
      if (mounted) {
        const info = await axios.get("https://acnhapi.com/v1a/sea/", {
        cancelToken: cancelTokenSource.token
      });
        await setSeaCrittersInfo(info.data);
    ...
    if (loaded){
        return(
            ...
            <div className="sea-critter-grid">
                {
                    SeaCrittersInfo.map((info) => {
                        return(<SeaCrittersInfoShown data={info} key={info['file-name']} location={hemisphere} month={month} hour={hour} />)
                    })
                }
            </div> 
        </div>
