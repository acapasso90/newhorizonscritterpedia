...
useEffect( async () => {
    let mounted = true;
    if (mounted) {
        const info = await axios.get("https://acnhapi.com/v1a/bugs/", {
        cancelToken: cancelTokenSource.token
      })
      await setbugInfo(info.data);
...
if (loaded){return(
    ...
    <div className="grid">
        {
            bugInfo.map((info) => {
                return(<BugInfoShown data={info} key={info['file-name']} location={hemisphere} month={month} hour={hour} />)
            })
        }
    </div> 
</div>
